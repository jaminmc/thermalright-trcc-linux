...
def _load_iokit() -> Optional[ctypes.CDLL]:
...
class SMCClient:
    ...
    def _read_key_raw(
        self, key: str,
    ) -> tuple[Optional[int], Optional[int], Optional[ctypes.Array]]:
    ...
    def read_key_float(self, key: str) -> Optional[float]:
    ...
    def read_fan_rpm(self, key: str) -> Optional[float]:
        data_type, data_size, buf = self._read_key_raw(key)
        return decode_fan_rpm_raw(data_type, data_size, buf)

    def read_key_uint32(self, key: str) -> Optional[int]:
