...
def _cfstr(s: str) -> Optional[ctypes.c_void_p]:
...
def _cfnumber_i32(v: int) -> Optional[ctypes.c_void_p]:
    assert _cf is not None
    buf = ctypes.c_int32(v)
    return _cf.CFNumberCreate(None, kCFNumberSInt32Type, ctypes.byref(buf))


def _matching_dict(page: int, usage: int) -> Optional[ctypes.c_void_p]:
    assert _cf is not None
...
def _normalize_hid_thermal_celsius(name: str, val: float) -> Optional[float]:
